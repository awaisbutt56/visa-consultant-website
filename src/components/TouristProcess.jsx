...
const TouristProcess = () => {
  ...
  return (
    <section className="relative py-16 bg-[#050505] px-6 overflow-hidden min-h-screen">
      ...
      <div className={`max-w-7xl mx-auto relative z-10 transition-all duration-700 ${selectedStep ? 'blur-sm scale-95 opacity-30' : ''}`}>
        ...
        <div className="text-center md:text-left mb-16 md:mb-24 flex flex-col md:flex-row md:items-end justify-between gap-8 md:gap-12">
          ...
          <div className="flex-1 flex flex-col items-center md:items-end space-y-4 md:space-y-6 pt-8 md:pt-0 border-t md:border-t-0 border-white/5 w-full max-w-lg md:max-w-xl mx-auto md:mx-0">
             ...
             <div className="w-full h-[2px] bg-gradient-to-r from-transparent via-amber-500/30 to-transparent relative overflow-hidden">
                <div className="absolute inset-y-0 left-0 w-20 bg-amber-500 animate-process-line"></div>
             </div>
        ...
        <div className="relative grid grid-cols-1 md:grid-cols-4 gap-12 md:gap-8">
          ...
          {steps.map((step, index) => (
            <div 
              ...
              className="group relative flex flex-col items-center md:items-start text-center md:text-left bg-[#0a0a0a] p-8 rounded-[2rem] border border-white/5 cursor-pointer transform-gpu transition-all duration-500 hover:border-amber-500/30 hover:bg-black/50 hover:-translate-y-3 shadow-2xl"
            >
              <div className="relative z-20 w-20 h-20 md:w-24 md:h-24 rounded-full bg-black border border-white/10 group-hover:border-amber-500/20 flex items-center justify-center text-amber-500 transition-all duration-500">
                 <span className="absolute -top-2 -right-2 bg-amber-500 text-black text-[10px] font-black px-2.5 py-1 rounded italic">
                    {step.id}
                 </span>
                 {step.icon}
              ...
              <div className="mt-8 flex items-center gap-3 transition-all duration-500 translate-y-4 group-hover:translate-y-0 opacity-0 group-hover:opacity-100 w-full justify-center md:justify-start">
                 ...
                 <span className="text-amber-500 font-black text-[10px] uppercase tracking-widest italic transition-colors">Detailed Specs</span>
              </div>
            </div>
          ))}
        </div>
      ...
      {/* --- OVERLAY WITH CUSTOM NEON SCROLLBAR --- */}
      {selectedStep && (
        <div className="fixed inset-0 z-[9999] bg-black/90 backdrop-blur-xl flex items-center justify-center p-4 md:p-6 animate-in fade-in duration-500">
          <div className="custom-scrollbar relative bg-[#0a0a0a] w-full max-w-4xl p-8 md:p-16 rounded-[2.5rem] md:rounded-[4rem] border border-amber-500/30 shadow-[0_0_100px_rgba(245,158,11,0.25)] overflow-hidden max-h-[90vh] overflow-y-auto transform-gpu animate-in slide-in-from-bottom duration-700">
            ...
            {/* Header Row: Icon + Mobile Button */}
            <div className="flex items-center justify-between md:justify-start gap-6 mb-10 relative z-10">
              <div className="w-20 h-20 md:w-24 md:h-24 rounded-full bg-amber-500/5 border border-amber-500/20 flex items-center justify-center text-amber-400">
                {selectedStep.icon}
              </div>

              {/* Close Button Next to Icon (Mobile Only) */}
              <button 
                onClick={() => setSelectedStep(null)}
                className="md:hidden flex items-center gap-2 px-5 py-3 bg-amber-500 text-black rounded-2xl font-black uppercase tracking-tighter text-[11px] active:scale-95 shadow-lg shadow-amber-500/40"
              >
                Close <X size={16} />
              </button>
            ...
            <div className="grid md:grid-cols-[1fr,2fr] gap-8 md:gap-12 items-start relative z-10">
                <div className="flex flex-col items-center md:items-start text-center md:text-left space-y-4">
                   <div className="space-y-3">
                      <span className="text-amber-500 font-black tracking-[0.3em] uppercase text-[10px] italic bg-amber-500/10 px-4 py-1.5 rounded-full inline-block">Protocol {selectedStep.id}</span>
                      <h4 className="text-3xl md:text-5xl font-black text-white uppercase italic tracking-tighter leading-tight">{selectedStep.title}</h4>
                   </div>
                </div>
                ...
                <div className="space-y-8 border-t md:border-t-0 md:border-l border-white/5 pt-8 md:pt-0 md:pl-12">
                   ...
                   <div className="space-y-4 pt-6 border-t border-white/5">
                      <h5 className="text-amber-500/50 font-black uppercase tracking-widest text-[11px] mb-4">Internal Checklist:</h5>
      ...
      <style jsx>{`
        ...
        .custom-scrollbar::-webkit-scrollbar-track {
          ...
          border-radius: 10px;
          margin: 30px; /* Thora padding scrollbar ke liye */
        }
